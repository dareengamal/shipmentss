...
app.get('/api/shippment/:order_id', async (req, res) => {
...
app.post('/api/shippment', async (req, res) => {
    try {
...
app.patch('/api/shippment', async (req, res) => {
    try {
...
app.delete('/api/shippment', async (req,res) => {
    try {
        console.log('[cancelShippment body]', req.body)
        const { order_id } = req.body;
        if (!order_id) return res.status(403).send('order_id is required');

        const shippment = await shippment.findOne({ order_id: order_id });
        if (!shippment) return res.status(403).send('Shipment does not exist');

        if(shippment.shippment_status === 'CREATED') {
            await shippment.updateOne({ order_id }, { shippment_status: 'CANCELED' });
            return res.status(200).send({ body: 'CANCELED', message: 'Your Shipment has been canceled' });
        }
        return res.status(200).send({ body: shippment.shipment_status, message: 'Cannot cancel this shipments' });
    }
    catch (e) {
        console.log('[cancelShippment] e', e)
    }
